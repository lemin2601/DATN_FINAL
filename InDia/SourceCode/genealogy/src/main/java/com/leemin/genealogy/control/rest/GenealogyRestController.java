...
public class GenealogyRestController {
    ...
    @PostMapping(value = "/rest/genealogy/delete/{idGenealogy}", produces = "application/json")
    public ResponseEntity<?> deleteGenealogy(
            @PathVariable(value = "idGenealogy")
                    Long  idGenealogy,
            Principal principal
    ) {
        if(!isAdminPermission(principal,idGenealogy)) return notHavePermisstion();
        userGenealogyRepository.deleteAllByGenealogyId(idGenealogy);
        return new ResponseEntity<>("success" , HttpStatus.OK);
    }



    private boolean isModPermisstion(Principal principal,Long idGenealogy){
        UserModel byEmail = userRepository.findByEmail(principal.getName());
        if(byEmail == null) return false;
        UserGenealogyModel byUserAndGenealogy_id = userGenealogyRepository.findTopByUserAndGenealogy_Id(byEmail, idGenealogy);
        return byUserAndGenealogy_id.getPermission().getName().toLowerCase().equals("admin") || byUserAndGenealogy_id.getPermission().getName().toLowerCase().equals("mod");
    }

    private boolean isAdminPermission(Principal principal,Long idGenealogy){
        UserModel byEmail = userRepository.findByEmail(principal.getName());
        if(byEmail == null) return false;
        UserGenealogyModel byUserAndGenealogy_id = userGenealogyRepository.findTopByUserAndGenealogy_Id(byEmail, idGenealogy);
